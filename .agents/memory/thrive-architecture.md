...
## One-team-per-coach invariant + onboarding auto-create
- A coach has exactly ONE team. This is enforced ONLY in application code, NOT by a DB constraint (no unique index on `teams.coach_user_id`). `POST /teams` is idempotent: if the coach already has a team it returns that team (200) instead of inserting a duplicate. Any NEW team-creation path must preserve this, or coaches end up with multiple teams.
- Coach onboarding (`pages/onboarding.tsx` finish()) auto-creates the team from the entered `teamName` right after persisting `userRole="coach"`, so coaches land on the dashboard with a ready invite code. The call is non-fatal (try/catch) — the Team page is the fallback.
